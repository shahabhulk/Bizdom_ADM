...
class Q3Helpers:
    ...
    @staticmethod
    def compute_employee_scores(category_lvl2_records, start_date, end_date, score_record, user, dept_id,
                                filter_type=None):
        """
        ...
        if score_record.score_name == "Expense":
            filtered_records = category_lvl2_records.filtered(
                lambda r: r.category_lvl2_selection and
                          r.category_lvl2_selection._name in selection_types
            )
        else:
            filtered_records = category_lvl2_records.filtered(
                lambda r: r.category_lvl2_selection and
        ...
        for cat_lvl2_rec in filtered_records:
            ...
            # Get name
            current_selection_type = selection_obj._name
            if current_selection_type == 'hr.employee':
                emp_name = selection_obj.name if hasattr(selection_obj, 'name') else "N/A"
            elif current_selection_type in ('product.category', 'product.product'):
                emp_name = selection_obj.name if hasattr(selection_obj, 'name') else "N/A"
            elif current_selection_type == 'fleet.vehicle.model.brand':
                emp_name = selection_obj.name if hasattr(selection_obj, 'name') else "N/A"
            else:  # utm.source
                emp_name = selection_obj.name if hasattr(selection_obj, 'name') else "N/A"
            ...
            if score_record.score_name == "Leads":
                ...
                source = selection_obj if selection_obj._name == 'utm.source' else None
            ...
            if score_record.score_name == "Labour" and filter_type in ["WTD", "MTD", "YTD", "CUSTOM", "Custom"]:
                ...
                if score_record.type == 'percentage':
                    ...
                    emp_data['min_value'] = single_rec.min_category_value_lvl2 or ''
                    emp_data['max_value'] = single_rec.max_category_value_lvl2 or ''
            else:
                emp_data['min_value'] = ''
                emp_data['max_value'] = ''
